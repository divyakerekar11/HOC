...
const GenerateInvoiceDialoge = ({ id, orderNo, dateOfOrder }: any) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {/* Button Section  */}
      <DialogTrigger>
        <PlusIcon className="h-[1.35rem] w-[1.75rem] p-1 hover:bg-[#013642] text-black hover:text-[white] border-0 rounded" />
      </DialogTrigger>
      {/* form Section  */}
      <DialogContent className="sm:max-w-[500px] text-[0.8rem]">
        <div className="flex justify-center text-xl">
          <FileTextIcon className="h-7 w-7 p-1 hover:bg-[#013642] text-black hover:text-[white] " />{" "}
          Generate Invoice
        </div>
