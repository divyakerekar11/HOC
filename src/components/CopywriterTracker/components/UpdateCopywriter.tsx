...
const UpdateCopywriter = ({ copywriterId }: any) => {
  const { copywriterUpdateData, fetchCopywriterUpdateData }: any =
    useEditorStore();
  ...
  return (
    <>
      {Array.isArray(copywriterUpdateData) &&
        copywriterId &&
        copywriterUpdateData.map((editor: any, index: number) => (
...
export default UpdateCopywriter;
